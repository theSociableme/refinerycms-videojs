  ...
  module Videos
    module PageVideos
      def has_many_page_videos
        has_many :video_pages, :as => :page, :order => 'position ASC', :class_name => "::Refinery::Videos::VideoPage"
        has_many :videos, :through => :video_pages, :order => 'position ASC', :class_name => "::Refinery::Videos::Video"
          ...
          def videos_attributes=(data)
            ...
            video_pages_to_delete = if ids_to_keep.empty?
              ...
              self.video_pages.where(
                Refinery::Videos::VideoPage.arel_table[:id].not_in(ids_to_keep)
              )
            end
        ...
        include Refinery::Videos::PageVideos::InstanceMethods
...
ActiveRecord::Base.send(:extend, Refinery::Videos::PageVideos)
